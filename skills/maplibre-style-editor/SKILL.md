...
### Step 1.5: ローカル参照ファイルを読み込む（必須）

**実装前に必ず `read_file` で以下の2ファイルを読み込む。** ここを省略してはならない。

```
- /home/hiro/.agents/skills/maplibre-style-editor/references/BEST_PRACTICES.md
- /home/hiro/.agents/skills/maplibre-style-editor/references/CAUTIONS.md
```

BEST_PRACTICES.md には式の選択・記法パターン（`line-gap-width` の使い方、`match` vs `case`、`step` vs `interpolate` など）が具体例とともに記載されている。  
CAUTIONS.md には `layout`/`paint` の区別・`source-layer` の制限など誤りやすい仕様が記載されている。  
これらの内容を確認してから実装に進む。
